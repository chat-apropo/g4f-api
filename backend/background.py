...
async def ai_respond(messages: list[dict], model: str, provider: ProviderType) -> str:
    ...
    if len(choices) == 0:
        raise CustomValidationError("No response from the provider", error={"messages": messages})
    ...
    async with semaphore:
        try:
            ...
            if hasattr(provider, "supported_models"):
                ...
                model = list(provider_and_models.all_working_providers_map[provider.__name__].supported_models)[0]
            else:
                model = "gpt-4"
...
async def update_working_providers():
    ...
    async with lock:
        ...
        print(f"Finished testing providers. Updating working providers to {len(now_working_providers)}")
        provider_and_models.update_model_providers(
            {provider_name: base_working_providers_map[provider_name] for provider_name in now_working_providers}
        )
